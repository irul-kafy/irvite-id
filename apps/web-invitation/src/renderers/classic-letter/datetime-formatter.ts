/**
 * Datetime Formatter for Classic Letter Event-Local Timezone-Neutral Ceremony Datetimes
 * Contract:
 ...
 * - If endDateTime < startDateTime -> fallback to start time only (no fabricated corrected end time).
 */
...
export function parseEventLocalDateTime(
  ...
  // Deterministic weekday using UTC to avoid browser timezone drift
  ...
  const formattedDateShort = `${dayStr} \u00b7 ${monthStr} \u00b7 ${yearStr}`;
 ...
 * - If endDateTime < startDateTime: renders start time only
 */
