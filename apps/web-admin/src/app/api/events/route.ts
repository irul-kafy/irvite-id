...
export async function GET() {
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: data.message || 'Error fetching events' }, { status: res.status });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      return NextResponse.json({ message: data.message || 'Error creating event' }, { status: res.status });
    }
