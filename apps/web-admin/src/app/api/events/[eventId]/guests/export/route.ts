...
export async function GET(
  ...
  try {
    ...
    const targetUrl = `${internalUrl}/events/${eventId}/guests/export?${queryParams.toString()}`;
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { message: data.message || 'Gagal mengekspor data tamu.' },
        { status: res.status },
      );
    }
    ...
    const contentDisposition =
      ...
      `attachment; filename="data-tamu.${queryParams.get('format')}"`;
