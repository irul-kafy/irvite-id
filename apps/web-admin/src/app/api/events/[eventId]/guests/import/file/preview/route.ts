...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${internalUrl}/events/${eventId}/guests/import/file/preview`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      return NextResponse.json({ message: data.message || 'Gagal memproses pratinjau berkas.' }, { status: res.status });
    }
