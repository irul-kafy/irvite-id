...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${internalUrl}/events/${eventId}/guests/${guestId}/invitation`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      return NextResponse.json({ message: data.message || 'Error generating invitation' }, { status: res.status });
    }
